...
with open('/mnt/p/name_dataset/US_Clean_TEST.txt','rt') as fi:
    nl_r = fi.readlines()
...
def A_FML(cfh_l)->str:
    try:
        ...
        for x in ln:
            ...
            if ln_1.strip().lower() in xs:
                for xln in xs:
                    ln_1_l.append(xln.strip().replace(chr(10),''))
        for x in sf:
            xs = [ll.lower() for ll in x]
            if ln_1.strip().lower() in xs:
                for xln in xs:
                    ln_1_l.append(xln.strip().replace(chr(10),''))
                    for x in ln_1_l:
                        cf_syn.append(str(f"%{x}%{cfh_l[1]}").replace('y','%').replace('an','%n').replace('en','%n'))
                        cf_syn.append(str(f"%{cfh_l[1]}%{x}%").replace('y','%').replace('an','%n').replace('en','%n'))
                    cf_q_check = len(cf_syn)
        print(cf_q_check)
        for x in cf_syn:
            cf_q = cf_q + x + '\n'
